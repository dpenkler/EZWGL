...
/**************************************************************
 *
 *  Scan converting a general polygon. Gouraud shaded without
 *  zbuffer.
 */
...
void CONVERT_GOURAUD_SHADED_POLYGON(vts, n)
     int  *vts, n;
{
  register PIXEL_TYPE_    *pixelptr, *rowptr;
#ifdef _ZBUFFERED_  
  ...
  bpl = BYTES_PER_LINE;
  ucptr = (unsigned char *)EZ_ImageData + y * bpl;
  rowptr = ((PIXEL_TYPE_ *)ucptr);
#ifdef _ZBUFFERED_  
  wwidth = WINDOW_WIDTH;
  ...
  if(EZ_ColorMode == EZ_CMAP_MODE )
    {
      for(; y < et.ymax ; y++, ucptr += bpl, rowptr = (PIXEL_TYPE_ *)ucptr
#ifdef _ZBUFFERED_  
	  , zrowptr += wwidth
#endif
	  )
			  ...
			  if( *zbufptr > zv)
			    {
			      *zbufptr = zv;
#endif
			      *pixelptr = (PIXEL_TYPE_) (EZ_ColorArrayFix[TO_COLOR_INDEX(cv)]);
#ifdef _ZBUFFERED_  
			    }
#endif
			}
	  ...
	  while(aetptr)
	    {
	      ...
	      if(inside && (naetptr))
		{
		  register int dz,dr,dg,db, zv,r,g,b;
		  register int x, x1;
#ifdef _DITHER_	  
		  register int ditherXY;
#endif
		  ...
		  if(x1 > x)
		    {  
		      ...
		      zbufptr  = zrowptr + x;
#endif
		      for( ; x <  x1; x++, pixelptr++, r += dr, g += dg, b += db
#ifdef _ZBUFFERED_  			
			     ,zv += dz,
			     zbufptr++
#endif
			  ...
			  if( *zbufptr > zv)
			    {
			      *zbufptr = zv;
			      ...
			      SET_PIXEL_VALUE_RGB_DITHER(*pixelptr, ditherXY, r, g, b);	
#else
			      SET_PIXEL_VALUE_RGB_FIX(*pixelptr, x, y, r, g, b);
#endif
#ifdef _ZBUFFERED_  
			    }
#endif
     ...
     int    *vts,  n;
{
  ...
  EZ_Edge                 *edgelist;
  EZ_Scanline             *scanline_allocated;
  int                     inside,bpl,ir,ig,ib;
  unsigned long           pixel_value,bgpix_value;
  unsigned char           *ucptr;
  ...
  bpl = BYTES_PER_LINE;
  ucptr = (unsigned char *)EZ_ImageData + bpl * y;
  rowptr = ((PIXEL_TYPE_ *)ucptr);
#ifdef _ZBUFFERED_  
  wwidth = WINDOW_WIDTH;
  zrowptr = EZ_Zbuf + y * wwidth;
  ...
  if(EZ_ColorMode == EZ_CMAP_MODE || EZ_DitheringFlag == 0)
      ...
      if(EZ_ColorMode == EZ_CMAP_MODE)
	{
	  ...
	  bgpix_value = EZ_ColorArrayFix[(int)EZ_BackRGBA[0]];
	}
      else 
	{
	  ...
	  SET_PIXEL_VALUE_RGB255_FIX(pixel_value,4,5,ir,ig,ib);
	  SET_PIXEL_VALUE_RGB255_FIX(bgpix_value,4,5,EZ_BackRGBA[0],EZ_BackRGBA[1], EZ_BackRGBA[2]);
	}
	  ...
	  )
	{
	  ...
	  while(aetptr)
	      ...
	      if(inside && naetptr)
		{
		  ...
		  register int dz,zv;
#endif
		  register int x, x0, x1,xm1;
	      
		  x = x0 = (aetptr->info).x;                /* the x (minor) coor to start  */
#ifdef _ZBUFFERED_  
		  zv = (aetptr->info).z;               /* the z value to start         */
#endif
		  x1 = (naetptr->info).x;
			  ...
			  if( *zbufptr > zv)
			    {
			      *zbufptr = zv;
#endif
			      *pixelptr = (PIXEL_TYPE_)(x == x0 || x == xm1) ? pixel_value : bgpix_value;
#ifdef _ZBUFFERED_  
			    }
#endif
			}
...
#ifdef _DITHER_
			      ditherXY = ditherY + (x & 0x0F);
			      SET_PIXEL_VALUE_RGB255_FLAT_DITHER(*pixelptr,ditherXY,dDr,dDg,dDb,dIr,dIg,dIb,
								 dIr1,dIg1,dIb1);
#else
			      if ((x==x0) || (x==x1) || (y==et.ymin)) {
			      fprintf(stderr,"spix");
				SET_PIXEL_VALUE_RGB255(*pixelptr,x,y,0,0,0);
			      } else {
			      fprintf(stderr,"npix");
				SET_PIXEL_VALUE_RGB255(*pixelptr,x,y,ir,ig,ib);
 			      }    
#endif
#ifdef _ZBUFFERED_  
			    }

...
void CONVERT_GOURAUD_SHADED_POLYGON(vts, n)
     int  *vts, n;
{
  register unsigned char  *pixelptr, *rowptr;
#ifdef _ZBUFFERED_  
  ...
  bpl = BYTES_PER_LINE;
  rowptr = (unsigned char *)EZ_ImageData + y * bpl;
#ifdef _ZBUFFERED_  
  wwidth = WINDOW_WIDTH;
...
#endif
		      pixelptr = rowptr + (x + x + x);
#ifdef _ZBUFFERED_  
		      zbufptr  = zrowptr + x;
#endif
		      for( ; x < x1; x++, pixelptr += 3, cv += dcv
#ifdef _ZBUFFERED_  			  
			  ,zv += dz,
			  zbufptr++
#endif
	      ...
	      if(inside && (naetptr))
		{
		  ...
		  if(x1 > x)
		    {  
		      ...
		      zbufptr  = zrowptr + x;
#endif
		      for( ; x <  x1; x++, pixelptr += 3, r += dr, g += dg, b += db
#ifdef _ZBUFFERED_  			
			     ,zv += dz,
			     zbufptr++
#endif
     ...
     int    *vts,  n;
{
  ...
  bpl = BYTES_PER_LINE;
  rowptr = (unsigned char *)EZ_ImageData + bpl * y;
#ifdef _ZBUFFERED_  
  wwidth = WINDOW_WIDTH;
  zrowptr = EZ_Zbuf + y * wwidth;
#endif
...
#ifdef _DITHER_
			      ditherXY = ditherY + (x & 0x0F);
			      SET_PIXEL_VALUE_RGB255_FLAT_DITHER(pixel_value,ditherXY,dDr,dDg,dDb,dIr,dIg,dIb,
								 dIr1,dIg1,dIb1);
#else

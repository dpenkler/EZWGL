     ...
     int vtx_bgn, vtx_end;
{
  ...
  int                     dx,dy, absdx2, absdy2, signdx;
  register int            dv;
		  ...
		  if( *zbufptr > zcoor)
		    {
		      *zbufptr = zcoor;
#endif
		      pixelvalue = (EZ_ColorArray[TO_COLOR_INDEX(ri)]);
		      XSetForeground(EZ_Display,EZ_MainGC,pixelvalue);
		  ...
		  if( *zbufptr > zcoor)
		    {
		      *zbufptr = zcoor;
#endif
		      SET_PIXEL_VALUE_RGB(pixelvalue, (xcoor), (ycoor), ri, g,b); 
		      XSetForeground(EZ_Display,EZ_MainGC,pixelvalue);
		      XDrawPoint(EZ_Display,EZ_Drawable,EZ_MainGC,xcoor,ycoor);
#ifdef _ZBUFFERED_  	  
		    }
     ...
     int vtx_bgn, vtx_end;
{
  ...
  if(dx !=0 || dy != 0)
      ...
      else
	{
	  ...
	  g =  clrs[1];
	  b =  clrs[2];
	  SET_PIXEL_VALUE_RGB(pixel_value,4,5, ir,g,b);
	}
     ...
     int  n;
{
  ...
  int                     dz,dri,dg,db;  /* error increaments              */ 
  int                     ri,g,b;  /* depth and color                */
  int                     loopcounter, dx,dy, absdx2, absdy2, signdx;
  register int            dv;
  unsigned long           pixelvalue;
  int                     itmp, lastx, lasty;
  int                     (*screenptr)[3] = EZ_VBuf.screen;
      ...
      if(dx !=0 || dy != 0)
	{
	  if(EZ_ColorMode == EZ_CMAP_MODE)
	  ...
	  else
	    {
	      ...
	      SET_PIXEL_VALUE_RGB255(pixel_value,4,5, ir, g, b);
	    }
	  XSetForeground(EZ_Display,EZ_MainGC,pixel_value);

     ...
     int   *vts, n;
{
  ...
  pixelstart = (PIXEL_TYPE_ *)EZ_RowImage->data; 
  /*
   * converting ...
   */
	      ...
	      if(inside && naetptr)
		{
		  ...
		  if(x1 > x)
...
#endif
			      pixelstart[pixel_index++] = (PIXEL_TYPE_) (EZ_ColorArrayFix[TO_COLOR_INDEX(cv)]);
#ifdef _ZBUFFERED_  
			    }
			  else if(pixel_index)
			    {
		      ...
		      if( *zbufptr > zv)
			{
			  *zbufptr = zv;
#endif
			  pixelstart[0] = (PIXEL_TYPE_) (EZ_ColorArrayFix[TO_COLOR_INDEX(cv)]);
			  XPutImage(EZ_Display,EZ_Drawable,EZ_MainGC, EZ_RowImage,
				    0,0,x, y, 1, 1);
#ifdef _ZBUFFERED_  
			}
			  ...
			  if( *zbufptr > zv)
			    {
			      *zbufptr = zv;
			      ...
			      SET_PIXEL_VALUE_RGB_DITHER(pixelstart[pixel_index++], ditherXY, r, g, b);  
#else
			      SET_PIXEL_VALUE_RGB_FIX(pixelstart[pixel_index++], x, y, r, g, b);
#endif
#ifdef _ZBUFFERED_  
			    }
			  else if(pixel_index)
...
#ifdef _DITHER_
			  ditherXY = ditherY + (x&0x0F);
			  SET_PIXEL_VALUE_RGB_DITHER(pixelstart[0], ditherXY, r, g, b);  
#else
			  SET_PIXEL_VALUE_RGB_FIX(pixelstart[0], x, y, r, g, b);
#endif
			  XPutImage(EZ_Display,EZ_Drawable,  EZ_MainGC, EZ_RowImage,
				    0,0,x, y, 1, 1);
#ifdef _ZBUFFERED_  
...
#ifdef _DITHER_
			      ditherXY = ditherY + (x &0x0F);
			      SET_PIXEL_VALUE_RGB255_FLAT_DITHER(pixelstart[pixel_index++],ditherXY,dDr,dDg,dDb,
								 dIr,dIg,dIb,dIr1,dIg1,dIb1);
#else
			      SET_PIXEL_VALUE_RGB255_FIX(pixelstart[pixel_index++],x,y,ir,ig,ib);
#endif
...
#ifdef _DITHER_
			  ditherXY = ditherY + (x &0x0F);
			  SET_PIXEL_VALUE_RGB255_FLAT_DITHER(pixelstart[0],ditherXY,dDr,dDg,dDb,
							     dIr,dIg,dIb,dIr1,dIg1,dIb1);
#else
			  SET_PIXEL_VALUE_RGB255_FIX(pixelstart[0],x,y,ir,ig,ib);
#endif
			  XPutImage(EZ_Display,EZ_Drawable, EZ_MainGC, EZ_RowImage,
				    0,0,x, y, 1, 1);
#ifdef _ZBUFFERED_  

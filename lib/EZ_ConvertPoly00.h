			  ...
			  if( *zbufptr > zv)
			    {
			      *zbufptr = zv;
			      ...
			      XPutPixel(ximage, x,y,
					(EZ_ColorArrayFix[TO_COLOR_INDEX(cv)]));
#ifdef _ZBUFFERED_  
			    }
#endif
			}
			  ...
			  if( *zbufptr > zv)
			    {
			      *zbufptr = zv;
			      ...
			      SET_PIXEL_VALUE_RGB_DITHER(pix_value, ditherXY, r, g, b);	
#else
			      SET_PIXEL_VALUE_RGB_FIX(pix_value, x, y, r, g, b);
#endif
			      XPutPixel(ximage, x,y, pix_value);
#ifdef _ZBUFFERED_  
			    }
#endif
			}
...
#ifdef _DITHER_
			      ditherXY = ditherY + (x & 0x0F);
			      SET_PIXEL_VALUE_RGB255_FLAT_DITHER(pix_value,ditherXY,dDr,dDg,dDb,dIr,dIg,dIb,
								 dIr1,dIg1,dIb1);
#else
			      SET_PIXEL_VALUE_RGB255(pix_value,x,y,ir,ig,ib);
#endif
			      XPutPixel(ximage, x,y, pix_value);
#ifdef _ZBUFFERED_  
			    }
		      ...
		      if( *zbufptr > zv)
			{
			  *zbufptr = zv;
			  ...
			  SET_PIXEL_VALUE_RGB255_FLAT_DITHER(pix_value,ditherXY,dDr,dDg,dDb,dIr,dIg,dIb,
							     dIr1,dIg1,dIb1);
#else
			  SET_PIXEL_VALUE_RGB255_FIX(pix_value,x,y,ir,ig,ib);
#endif
			  XPutPixel(ximage, x,y, pix_value);
#ifdef _ZBUFFERED_  
			}

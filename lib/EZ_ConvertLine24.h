     ...
     int  vtx_bgn, vtx_end;
{
  ...
  int                     dx,dy, absdx2, absdy2, signdx,s3x, bpl;
  register int            dv;
		  ...
		  if( *zbufptr > zcoor)
		    {
		      *zbufptr = zcoor;
#endif
		      pixel_value = (EZ_ColorArrayFix[TO_COLOR_INDEX(ri)]);
		  ...
		  if( *zbufptr > zcoor)
		    {
		      *zbufptr = zcoor;
#endif
		      SET_PIXEL_VALUE_RGB(pixel_value, (xcoor), (ycoor), ri, g,b);
		      pixelptr[0] =pv4[0]; pixelptr[1] =pv4[1];
		      pixelptr[2] =pv4[2];		      
#ifdef _ZBUFFERED_  	  
		    }
     ...
     int n;
{
  ...
  int                     dz,dri,dg,db;  /* error increaments              */ 
  int                     ri,g,b;  /* depth and color                */
  int                     loopcounter, dx,dy, absdx2, absdy2, signdx, s3x, bpl;
  register int            dv;
  int                     itmp, lastx, lasty;
  int                     (*screenptr)[3] = EZ_VBuf.screen;
  int                     (*colorptr)[4] = EZ_VBuf.rgba;

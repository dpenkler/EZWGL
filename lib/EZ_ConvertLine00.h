     ...
     int  vtx_bgn, vtx_end;
{
  ...
  int                     dx,dy, absdx2, absdy2, signdx, wwidth;
  register int            dv;
		  ...
		  if( *zbufptr > zcoor)
		    {
		      *zbufptr = zcoor;
#endif
		      XPutPixel(ximage, xcoor, ycoor,
				(EZ_ColorArrayFix[TO_COLOR_INDEX(ri)]));
#ifdef _ZBUFFERED_  	  
		    }
#endif
		  ...
		  if( *zbufptr > zcoor)
		    {
		      *zbufptr = zcoor;
#endif
		      SET_PIXEL_VALUE_RGB(pixvalue,  (xcoor), (ycoor), ri, g,b);
		      XPutPixel(ximage,  (xcoor), (ycoor), pixvalue);
#ifdef _ZBUFFERED_  	  
		    }
#endif
		  if( ycoor == lasty)     /* finished         */
     ...
     int n;
{
  ...
  int                     dz,dri,dg,db;  /* error increaments              */ 
  int                     ri,g,b;  /* depth and color                */
  int                     loopcounter, dx,dy, absdx2, absdy2, signdx,wwidth;
  register int            dv;
  int                     itmp, lastx, lasty;
  int                     (*screenptr)[3] = EZ_VBuf.screen;
  int                     (*colorptr)[4] = EZ_VBuf.rgba;
	      ...
	      if(absdy2>= absdx2)                  /* y majored         */
		{
		  ...
		  for( ; ; )
		      ...
		      if( *zbufptr > zcoor)
			{
			  *zbufptr = zcoor;
#endif
			  SET_PIXEL_VALUE_RGB(pix_value, (xcoor), (ycoor), ri, g,b);	
			  XPutPixel(ximage, xcoor,ycoor, pix_value);
#ifdef _ZBUFFERED_  	  
			}
#endif
		      if( ycoor == lasty)     /* finished         */
...
#ifdef _ZBUFFERED_  	  
  register int            *zbufptr;
  int                     zcoor, dz;     /* depth and its error increament */ 
#endif
